...
{  
	...
	{
		/*  (2-)  To map seq1 in reg seqr3 which is reg 2 in array  */
		L_u8_Index1 = 2 -( Copy_eSequenceNum /6) ;
		/* the position of seq in reg */
...
// Send size of scan and the two element to save in
void MADC_RegulerScan(u16 * Copy_u16PtrData , u8* Copy_u8TotalConversion_nu)
{
	#if (1)
	u8 L_u8Size=0;
	// if seq num > 0 && < 16
	...
	{
		// once  scan one chanel so one seq
		...
		{
			while (MADC1 ->SR_ .SR.EOC ==0);
			Copy_u16PtrData [ L_u8Size]= MADC1 -> DR;
		}
