...
void MEXTI_vPortConfig(EXTI_PORT_Num Copy_ePortNum,EXTI_LINE_t Copy_eLineNum)
{
	...
	if(L_LineNum<=3)
	{
		MSYSCFG->EXTICR[0]|=((Copy_ePortNum<<0)<<(4U * L_LineNum ));
	}
	else if(L_LineNum<=7)
	{
		L_LineNum-=4;
		MSYSCFG->EXTICR[1]|=((Copy_ePortNum<<0)<<(4U * L_LineNum ));
	}
	else if(L_LineNum<=11)
	{
		L_LineNum-=8;
		MSYSCFG->EXTICR[2]|=((Copy_ePortNum<<0)<<(4U * L_LineNum ));
	}
	else if(L_LineNum<=15)
	{
		L_LineNum-=12;
		MSYSCFG->EXTICR[3]|=((Copy_ePortNum<<0)<<(4U * L_LineNum ));
	}
	else 
	{
		/*return error*/
